...
echo 'make distclean 2>/dev/null 1>/dev/null'
make distclean 2>/dev/null 1>/dev/null

# REGENERATE BUILD FILES IF NECESSARY OR REQUESTED
if [[ ! -f "${BASEDIR}"/src/"${LIB_NAME}"/configure ]] || [[ ${RECONF_openssl} -eq 1 ]]; then
  echo 'autoreconf_library "${LIB_NAME}" 1>>"${BASEDIR}"/build.log 2>&1 || return 1'
...
echo 'make -j$(get_cpu_count) build_sw || return 1'
make -j$(get_cpu_count) build_sw || return 1

echo 'make install_sw install_ssldirs || return 1'
